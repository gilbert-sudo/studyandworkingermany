    ...
    <div className={`fixed inset-0 z-[100] flex items-center justify-center transition-opacity duration-300 ${isAnimating ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className={`relative w-full max-w-md mx-4 bg-white dark:bg-gray-900 rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-800 overflow-hidden transform transition-all duration-300 ease-out ${isAnimating ? 'scale-100 opacity-100 translate-y-0' : 'scale-95 opacity-0 translate-y-4'}`}>
        ...
        <div className="p-8 relative z-10 mt-4">
          ...
          <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); /* Add auth logic here */ }}>
            {!isLogin && (
            ...
            <button type="submit" className="w-full py-3 px-4 bg-gradient-to-r from-brand-red to-red-600 text-white rounded-xl font-semibold shadow-lg shadow-brand-red/30 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 mt-4">
          ...
          <div className="mt-8 text-center">
            <p className="text-sm text-gray-500 dark:text-gray-400">
              ...
              <button type="button" onClick={() => dispatch(setAuthModalMode(isLogin ? 'signup' : 'login'))} className="font-semibold text-brand-red hover:text-red-700 transition-colors">
